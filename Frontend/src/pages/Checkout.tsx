...
const Checkout = () => {
  ...
  const { data: deliveryInfo, isLoading: isDeliveryLoading } =
    useQuery<DeliveryInfo>({
      ...
      queryFn: async () => {
        if (!coordsToUse)
          return { fee: 50, message: "Default fee", canDeliver: true };
  ...
  const orderMutation = useMutation({
    ...
    onSuccess: async (response) => {
      ...
      if (paymentMethod === "esewa" || paymentMethod === "khalti") {
        try {
          const { data: paymentInit } = await api.post("/payment/initiate", {
            orderId: order._id,
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (deliveryInfo && !deliveryInfo.canDeliver) {
      toast.error(
        "Sorry, delivery may not be available to your location, but we'll try!.",
      );
      return;
    }
                ...
                <CardContent className="space-y-4">
                  ...
                  <div className="space-y-2">
                      ...
                      <div className="flex items-center gap-2 text-xs">
                        ...
                        {deliveryCoords && !isGeocoding && (
                          <span className="flex items-center gap-1 text-green-600">
                            ...
                            📍 Location detected (
                            {deliveryCoords.lat.toFixed(4)},{" "}
                            {deliveryCoords.lng.toFixed(4)})
                          </span>
                        )}
                      </div>
                      {/* GPS fallback button */}
                    ...
                    <p className="text-xs text-muted-foreground">
                      💡 Your delivery address is used to find nearby chefs and
                      calculate delivery fee accurately.
                    </p>
                  </div>
