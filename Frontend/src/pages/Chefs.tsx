...
const fetchChefWithMenu = async (
  chefId: string,
): Promise<{ chef: Chef; menuItems: MenuItem[] }> => {
...
function ChefCard({ chef, onClick }: { chef: Chef; onClick: () => void }) {
  return (
    <Card
      className="overflow-hidden cursor-pointer hover-lift transition-all duration-300 hover:border-primary hover:shadow-lg"
      onClick={onClick}
    >
      <div className="relative h-48 bg-gradient-to-br from-orange-50 to-amber-100 overflow-hidden">
        {chef.photo ? (
          ...
          <div className="w-full h-full flex items-center justify-center">
            <ChefHat className="h-20 w-20 text-orange-200" />
          </div>
        )}
        ...
        <div className="absolute top-3 left-3 bg-green-500 text-white rounded-full px-2 py-0.5 text-xs font-bold">
          Active
        </div>
      </div>
      <CardContent className="p-4">
        ...
        <div className="flex items-center gap-1 mt-2 text-muted-foreground flex-wrap">
          ...
          <span className="text-xs">
            {typeof chef.location === "object"
              ? (chef.location as any).city || "Local"
              : chef.location}
          </span>
    ...
    <Card className="overflow-hidden hover-lift transition-all duration-200 hover:border-primary">
      ...
      <CardContent className="p-3">
        ...
        <div className="flex items-center gap-3 mt-1 text-xs text-muted-foreground">
          ...
          {item.spiceLevel && (
            <span
              className={`px-1.5 py-0.5 rounded-full font-semibold ${spiceBadge[item.spiceLevel] ?? ""}`}
            >
              {item.spiceLevel}
            </span>
          )}
...
function ChefProfile({
  ...
  const [filter, setFilter] = useState<
    "all" | "veg" | "non-veg" | "dessert" | "beverage"
  >("all");
    ...
    <div className="animate-fade-in">
      ...
      <div className="relative rounded-2xl overflow-hidden mb-8 bg-gradient-to-r from-stone-900 via-orange-950 to-stone-900">
        <div
          ...
          style={{
            backgroundImage:
              "radial-gradient(circle at 20% 50%, #f97316 0%, transparent 60%), radial-gradient(circle at 80% 50%, #ea580c 0%, transparent 60%)",
          }}
        />
        ...
        {chef.bio && (
          <div className="px-8 pb-6 text-white/80 text-sm italic border-t border-white/10 pt-4">
            "{chef.bio}"
          </div>
        )}
      </div>
...
const ChefsPage = () => {
  ...
  const sortedChefs = [...chefsWithDistance].sort(
      ...
      <div className="container mx-auto px-4">
        ...
        {!isLoading && (!chefs || chefs.length === 0) && (
          <div className="text-center py-20 text-muted-foreground">
            ...
            <p className="text-sm">
              Add chefs from the admin panel to get started.
            </p>
          </div>
        )}
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
            {sortedChefs.map((chef) => (
              <ChefCard
                key={chef._id}
                chef={chef}
                onClick={() => setSelectedChefId(chef._id)}
              />
            ))}
          </div>
        )}
