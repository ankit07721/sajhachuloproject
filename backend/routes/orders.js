  ...
  try {
    ...
    for (const item of items) {
      ...
      if (!menuItem || !menuItem.isAvailable) {
        return res
          .status(400)
          .json({ success: false, message: `Item is not available.` });
      }
      ...
      orderItems.push({
        ...
        chefName: isChef
          ? `${menuItem.createdBy.firstName} ${menuItem.createdBy.lastName}`
          : null,
      });
    }
    ...
    console.log(
      `[ORDER CREATED] ${savedOrder.orderNumber} | Chef: ${primaryChef?.chefName || "Admin"} | Distance: ${deliveryDistance}km`,
    );
    ...
    try {
      ...
      const filteredOrders = orders.map((order) => ({
        ...
        myItems: order.items.filter(
          (item) => item.chef && item.chef.toString() === req.user.id,
        ),
      }));
      ...
      res
        .status(500)
        .json({ success: false, message: "Failed to fetch chef orders" });
    }
  },
);
